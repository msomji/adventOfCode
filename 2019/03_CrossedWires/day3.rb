...
class WireOverlap
    ...
    def find_intersections coords1, coords2
        coorslength = coords1.length
         coords1.select.with_index do |coord ,index|
            # p "doing #{index} of #{coorslength}"
              coords2.any? {|c| c == coord} 
         end
    end
   ...
   def distance_from_start coords
    ...
    def initialize wire1, wire2
        @wire1 = wire1
        @wire2 = wire2
        @coords1 = findAllCoords(wire1)
        @coords2 = findAllCoords(wire2)
        p @intersections = find_intersections(@coords1.dup, @coords2.dup)
        p @intersections.map.with_index { |i, index| [@coords1.find_index(i), @coords2.find_index(i)].inject(0, :+) }.sort()
...
# test1 = ["R98","U47","R26","D63","R33","U87","L62","D20","R33","U53","R51"]
# test2 = ["U98","R91","D20","R16","D67","R40","U7","R15","U6","R7"]
WireOverlap.new(WIRE1, WIRE2)


# results 
# intersections = [[0, 0], [-1422, -957], [-1527, -957], [-1527, -989], [-1422, -989], [-1305, -901], [-1724, -687], [-1834, -613], [-1609, 441], [-1569, 689], [-1569, 484], [-1774, 484], [-2264, 498], [-2291, 594], [-2299, 594], [-2396, 1011], [-2396, 1037], [-1678, 1850]]
# distances = [0, 2050, 2053, 2206, 2258, 2258, 2379, 2411, 2411, 2447, 2484, 2516, 2762, 2885, 2893, 3407, 3433, 3528]
