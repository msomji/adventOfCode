...
class SpacePolice {
    ...
    draw(canvas) {
        let joined = canvas.map(c => c.join(''))
        console.clear();
        console.log(joined)
    }
    paint(initial = BLACK, initialCanvas) {
        let paintedCoords = {
            '50,50': initial
        };
        ...
        let run = new Intcode(currentProgram).process().next(nextInput)
            while(run.next !== undefined) {
                canvas[location[1]][location[0]] = nextInput === BLACK ? '#' : ' '
                this.draw(canvas)
                ...
                nextInput = paintedCoords[location] || BLACK
                run = run.next(nextInput)
            }

        return {paintedCoords, canvas}
    }
}
...
let path= new SpacePolice(INPUT).paint(WHITE, canvas)
console.log(Object.keys(path.paintedCoords).length)