...
class Intcode {
    ...
    process() {
        ...
        while (!halted) {
            ...
            let param1 = param1Mode == 0 ? this.getPositionMode(this.program, this.position+1) : this.getImmidiateMode(this.program, this.position+1)
            let param2 = param2Mode == 0 ? this.getPositionMode(this.program, this.position+2) : this.getImmidiateMode(this.program, this.position+2)
            let param3 = param3Mode == 0 ? this.getPositionMode(this.program, this.position+3) : this.getImmidiateMode(this.program, this.position+3)
            ...
            switch (opcode) {
                ...
                case 2: 
                    ...
                    break;
                case 3: 
                    this.program[this.getImmidiateMode(this.program, this.position+1)] = 12 //input
                    this.position+=2
                    break;
                case 4: 
                    console.log(`printed: ${this.program[this.getImmidiateMode(this.program, this.position+1)]}`)
                    this.position+=2
                    break;
                case 5: //jum if true
                    if(param1 !== 0) {
                ...
                case 6: // jump if false
                    ...
                    break;
                case 7: 
                    this.position[param3]  = (param1 < param2) ? 1: 0
                    break;
                case 8: 
                    this.position[param3]  = (param1 < param2) ? 1: 0
                    break;
...
// let foo = new Intcode([1,1,1,4,99,5,6,0,99])
INPUT = [3,0,4,0,99] // outputs the input

INPUT = [1002,4,3,4,33]
// INPUT = [1101,100,-1,4,0]
let foo = new Intcode(INPUT)
console.log(`returned: ${foo.process()}`)